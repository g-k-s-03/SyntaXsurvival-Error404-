...
def overview(
    ...
    active_requests = (
        ...
        .filter(BloodRequest.status.in_([RequestStatus.submitted, RequestStatus.matched, RequestStatus.alerted, RequestStatus.accepted]))
        .count()
    )
...
def broadcast(
    ...
    _require_admin(user)
    # MVP: we only log + return recipient estimate.
